...
* Change "?" to NA 
* Convert categorical columns to binary values: 0 and 1.
...
We need all numerical values to apply the first 3 methods, so create new dataset which containts only these variables.
...
In the last function, mice, we impute missing values once (df4) and five times (df5) to achieve better results.
...
```{r fig3, fig.height = 6, fig.width = 6, fig.align = 'center', warning=FALSE, message=FALSE, echo=FALSE}
ggplot(na.omit(df_all), aes(x = LiverFirm, fill = method)) +
  scale_fill_brewer(palette = "Set2") +
  geom_bar(position = "dodge")
```

```{r fig2, fig.height = 7, fig.width = 7, fig.align = 'center', warning=FALSE, message=FALSE, echo=FALSE}
...
```{r, fig.height = 6, fig.width = 6, fig.align = 'center', warning=FALSE, message=FALSE, echo=FALSE}
...
```{r, fig.height = 6, fig.width = 10, fig.align = 'center', warning=FALSE, message=FALSE, echo=FALSE}
...
```{r, fig.height = 10, fig.width = 10, fig.align = 'center', warning=FALSE, message=FALSE, echo=FALSE}
...
According to the correlation matrix we have low correlation for all pair of features (< 0.41). Furthermore, as can be seen on the scatter plots, combinations of Albumin and Protime with other attributes seem to be good at distinguishing between classes.

We can also observe two outliers (people with high level of Bilirubin).

```{r fig_corr, fig.height = 10, fig.width = 10, fig.align = 'center', echo=FALSE}
...
```{r fig_corr2, fig.height = 10, fig.width = 10, fig.align = 'center', echo=FALSE}
...
* Next fields seems to be important: Antivirals, Fatigue, LiverBig, Albumin,  Bilirubin, and Protime;
...
We have the same values in variable Class in datasets for all imputing methods, so we can use the same inTrain dataset.
...
To solve class imbalance problem we decided to use Majority Weighted Minority Oversampling Technique.
...
We then decided to construct the model using important features. We also wanted to check if our initial insights were correct and fitted nine models using all combinations of Albumin and Protime with other attributes.

Selected features for the model:

1. Albumin, Bilirubin

2. Albumin, AlkPhosphate 
...
7. Protime, AlkPhosphate

8. Protime, Sgot

9. Albumin, Age

10. Sex, Malaise, Anorexia, SpleenPalpable, Spiders, Bilirubin, Albumin, Protime
...
lr_metrices1 <- cbind(metric=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "Precision"),lr_metrices1)
...
lr_metrices2 <- cbind(metric=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "Precision"),lr_metrices2)
...
lr_metrices3 <- cbind(metric=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "Precision"),lr_metrices3)
...
lda_metrices1 <- cbind(metric=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "Precision"),lda_metrices1)
...
lda_metrices2 <- cbind(metric=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "Precision"),lda_metrices2)
...
As can be observed in the tables below, for the first model (Bilirubin-Protime) knn impute is the best. For the other model there aren't any differences between methods.
To conclude, once again the Bilirubin and Protime attributes are the most efficient in distinguishing classes. The differences between LDA and QDA depends on features selected to build the model. 
```{r echo=FALSE}
model.1.1 <- qda(Class~Albumin+Bilirubin, data=train.num1) 
model.1.2 <- qda(Class~Albumin+Bilirubin, data=train.num2) 
model.1.3 <- qda(Class~Albumin+Bilirubin, data=train.num3) 

model.5.1 <- qda(Class~Protime+Age, data=train.num1) 
model.5.2 <- qda(Class~Protime+Age, data=train.num2) 
model.5.3 <- qda(Class~Protime+Age, data=train.num3) 

model.6.1 <- qda(Class~Protime+Bilirubin, data=train.num1) 
model.6.2 <- qda(Class~Protime+Bilirubin, data=train.num2) 
model.6.3 <- qda(Class~Protime+Bilirubin, data=train.num3) 
```
```{r, echo=FALSE}
p_thr = 0.167
Albumin.Bilirubin.1 <- metrices(lda_pred(model.1.1, test.num1, p_thr), test1$Class)
Albumin.Bilirubin.2 <- metrices(lda_pred(model.1.2, test.num2, p_thr), test2$Class)
Albumin.Bilirubin.3 <- metrices(lda_pred(model.1.3, test.num3, p_thr), test3$Class)

Protime.Age.1 <- metrices(lda_pred(model.5.1, test.num1, p_thr), test1$Class)
Protime.Age.2 <- metrices(lda_pred(model.5.2, test.num2, p_thr), test2$Class)
Protime.Age.3 <- metrices(lda_pred(model.5.3, test.num3, p_thr), test3$Class)

Protime.Bilirubin.1 <- metrices(lda_pred(model.6.1, test.num1, p_thr), test1$Class)
qda.pb.1 <- Protime.Bilirubin.1
Protime.Bilirubin.2 <- metrices(lda_pred(model.6.2, test.num2, p_thr), test2$Class)
Protime.Bilirubin.3 <- metrices(lda_pred(model.6.3, test.num3, p_thr), test3$Class)

qda_metrices1 <- data.frame(Protime.Bilirubin.1, Protime.Bilirubin.2, Protime.Bilirubin.3)
qda_metrices1 <- round(qda_metrices1,2)
qda_metrices1 <- cbind(metric=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "Precision"),qda_metrices1)

qda_metrices2 <- data.frame(Albumin.Bilirubin.1, Albumin.Bilirubin.2, Albumin.Bilirubin.3)
qda_metrices2 <- round(qda_metrices2,2)
qda_metrices2 <- cbind(metric=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "Precision"),qda_metrices2)

qda_metrices3 <- data.frame(Protime.Age.1, Protime.Age.2, Protime.Age.3)
qda_metrices3 <- round(qda_metrices3,2)
qda_metrices3 <- cbind(metric=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "Precision"),qda_metrices3)
...
The next step is to build the logistic regression model, excluding variables that are unnecessary based on the summary and irrelevant from the EDA.

The obtained models:

1. Excluding Age, Anorexia, Bilirubin

2. Excluding Age, Steroid, AlkPhosphate, Sgot
...
In the final step, convert probabilities to class labels using a cutoff of 0.5.
...
```{r, echo=FALSE}
cm1 <- confusionMatrix(table(pred.labels1, real.labels), positive = "1")
cm2 <- confusionMatrix(table(pred.labels2, real.labels), positive = "1")
cm3 <- confusionMatrix(table(pred.labels3, real.labels), positive = "1")
cm4 <- confusionMatrix(table(pred.labels4, real.labels), positive = "1")
cm5 <- confusionMatrix(table(pred.labels5, real.labels), positive = "1")
cm6 <- confusionMatrix(table(pred.labels6, real.labels), positive = "1")
cm7 <- confusionMatrix(table(pred.labels7, real.labels), positive = "1")
cm8 <- confusionMatrix(table(pred.labels8, real.labels), positive = "1")
cm9 <- confusionMatrix(table(pred.labels9, real.labels), positive = "1")
```

```{r, echo=FALSE}
logit1 <- confusion.matrix.results(cm1)
logit2 <- confusion.matrix.results(cm2)
logit3 <- confusion.matrix.results(cm3)
logit4 <- confusion.matrix.results(cm4)
logit5 <- confusion.matrix.results(cm5)
logit6 <- confusion.matrix.results(cm6)
logit7 <- confusion.matrix.results(cm7)
logit8 <- confusion.matrix.results(cm8)
logit9 <- confusion.matrix.results(cm9)
```

```{r, echo=FALSE, warning=FALSE}
df1.num <- to_numeric1(df1[-21])
df2.num <- to_numeric1(df2[-21])
df5.num <- to_numeric1(df5[-21])

er1 <- errorest(Class~.-Class, data=df1.num, model=glm, estimator="cv")
er2 <- errorest(Class~.-Class, data=df2.num, model=glm, estimator="cv")
er3 <- errorest(Class~.-Class, data=df5.num, model=glm, estimator="cv")

er4 <- errorest(Class~.-Class-Age-Anorexia-Bilirubin, data=df1.num, model=glm, estimator="cv")
er5 <- errorest(Class~.-Class-Age-Anorexia-Bilirubin, data=df2.num, model=glm, estimator="cv")
er6 <- errorest(Class~.-Class-Age-Anorexia-Bilirubin, data=df5.num, model=glm, estimator="cv")

er7 <- errorest(Class~.-Class-Age-Steroid-AlkPhosphate-Sgot, data=df1.num, model=glm, estimator="cv")
er8 <- errorest(Class~.-Class-Age-Steroid-AlkPhosphate-Sgot, data=df2.num, model=glm, estimator="cv")
er9 <- errorest(Class~.-Class-Age-Steroid-AlkPhosphate-Sgot, data=df5.num, model=glm, estimator="cv")
```

```{r, echo=FALSE}
logit1 <- append(logit1, er1$error)
logit2 <- append(logit2, er2$error)
logit3 <- append(logit3, er3$error)
logit4 <- append(logit4, er4$error)
logit5 <- append(logit5, er5$error)
logit6 <- append(logit6, er6$error)
logit7 <- append(logit7, er7$error)
logit8 <- append(logit8, er8$error)
logit9 <- append(logit9, er9$error)
```

```{r, echo=FALSE, warning=FALSE}
er1 <- errorest(Class~.-Class, data=df1.num, model=glm, estimator="boot")
er2 <- errorest(Class~.-Class, data=df2.num, model=glm, estimator="boot")
er3 <- errorest(Class~.-Class, data=df5.num, model=glm, estimator="boot")

er4 <- errorest(Class~.-Class-Age-Anorexia-Bilirubin, data=df1.num, model=glm, estimator="boot")
er5 <- errorest(Class~.-Class-Age-Anorexia-Bilirubin, data=df2.num, model=glm, estimator="boot")
er6 <- errorest(Class~.-Class-Age-Anorexia-Bilirubin, data=df5.num, model=glm, estimator="boot")

er7 <- errorest(Class~.-Class-Age-Steroid-AlkPhosphate-Sgot, data=df1.num, model=glm, estimator="boot")
er8 <- errorest(Class~.-Class-Age-Steroid-AlkPhosphate-Sgot, data=df2.num, model=glm, estimator="boot")
er9 <- errorest(Class~.-Class-Age-Steroid-AlkPhosphate-Sgot, data=df5.num, model=glm, estimator="boot")
```
...
```{r, echo=FALSE}
logit_statistics <- data.frame(logit1, logit2, logit3, logit4, logit5, logit6, logit7, logit8, logit9)
logit_statistics <- round(logit_statistics,2)
logit_statistics <- cbind(name=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "10-fold CV error", "Bootstrap error"), logit_statistics)
```
...
knn_metrices <- cbind(metric=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "Precision", "k"),knn_metrices)
...
```{r echo=FALSE}
knn_metrices <- data.frame(all_metrics)
colnames(knn_metrices) <- c("All","Albumin.Ascites.Protime", "Albumin.Ascites.Malaise", "Albumin.Ascites.Bilirubin", "Albumin.Protime.Malaise", "Albumin.Protime.Bilirubin","Albumin.Malaise.Bilirubin","Ascites.Protime.Malaise","Ascites.Protime.Bilirubin", "Ascites.Malaise.Bilirubin", "Protime.Malaise.Bilirubin")
...
We will finally compare different imputation methods. Protime-Bilirubin for knn imputation method is again the best model.
...
### Method
We can use both numeric and categorical data. Let's specify model formula. We use all attributes and also model without variables that are useless from EDA (Age, Steroid, AlkPhosphate, Sgot).
...
Let's visualize all the trees. They are pretty different: from a tree with only 2 splits to a tree with many splits.
...
The last step is to predict class labels.
...
```{r, echo=FALSE}
tree1 <- confusion.matrix.results(cm1)
tree2 <- confusion.matrix.results(cm2)
tree3 <- confusion.matrix.results(cm3)
tree4 <- confusion.matrix.results(cm4)
tree5 <- confusion.matrix.results(cm5)
tree6 <- confusion.matrix.results(cm6)
tree7 <- confusion.matrix.results(cm7)
tree8 <- confusion.matrix.results(cm8)
```

```{r, echo=FALSE}
df1 <- to_categorical(df1)
df2 <- to_categorical(df2)
df5 <- to_categorical(df5)
df <- to_categorical(df)
```

```{r, echo=FALSE, warning=FALSE}
my.pred <- function(object, newdata)  predict(object, newdata=newdata, type="class")
my.rpart <- function(formula1, data1) rpart(formula=formula1,data=data1, minsplit=5)

er1 <- errorest(mod1, data=df1, model=my.rpart, estimator="cv", predict=my.pred)
er2 <- errorest(mod1, data=df2, model=my.rpart, estimator="cv", predict=my.pred)
er3 <- errorest(mod1, data=df5, model=my.rpart, estimator="cv", predict=my.pred)
er4 <- errorest(mod1, data=df, model=my.rpart, estimator="cv", predict=my.pred)

er5 <- errorest(mod2, data=df1, model=my.rpart, estimator="cv", predict=my.pred)
er6 <- errorest(mod2, data=df2, model=my.rpart, estimator="cv", predict=my.pred)
er7 <- errorest(mod2, data=df5, model=my.rpart, estimator="cv", predict=my.pred)
er8 <- errorest(mod2, data=df, model=my.rpart, estimator="cv", predict=my.pred)
```

```{r, echo=FALSE}
tree1 <- append(tree1, er1$error)
tree2 <- append(tree2, er2$error)
tree3 <- append(tree3, er3$error)
tree4 <- append(tree4, er4$error)
tree5 <- append(tree5, er5$error)
tree6 <- append(tree6, er6$error)
tree7 <- append(tree7, er7$error)
tree8 <- append(tree8, er8$error)
```

```{r, echo=FALSE, warning=FALSE}
er1 <- errorest(mod1, data=df1, model=my.rpart, estimator="boot", predict=my.pred)
er2 <- errorest(mod1, data=df2, model=my.rpart, estimator="boot", predict=my.pred)
er3 <- errorest(mod1, data=df5, model=my.rpart, estimator="boot", predict=my.pred)
er4 <- errorest(mod1, data=df, model=my.rpart, estimator="boot", predict=my.pred)

er5 <- errorest(mod2, data=df1, model=my.rpart, estimator="boot", predict=my.pred)
er6 <- errorest(mod2, data=df2, model=my.rpart, estimator="boot", predict=my.pred)
er7 <- errorest(mod2, data=df5, model=my.rpart, estimator="boot", predict=my.pred)
er8 <- errorest(mod2, data=df, model=my.rpart, estimator="boot", predict=my.pred)
```
...
```{r, echo=FALSE}
tree_statistics <- data.frame(tree1, tree2, tree3, tree4, tree5, tree6, tree7, tree8)
tree_statistics <- round(tree_statistics,2)
tree_statistics <- cbind(name=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "10-fold CV error", "Bootstrap error"), tree_statistics)
```
...
```{r, echo=FALSE}
btree1 <- confusion.matrix.results(cm1)
btree2 <- confusion.matrix.results(cm2)
btree3 <- confusion.matrix.results(cm3)
btree4 <- confusion.matrix.results(cm4)
btree5 <- confusion.matrix.results(cm5)
btree6 <- confusion.matrix.results(cm6)
btree7 <- confusion.matrix.results(cm7)
btree8 <- confusion.matrix.results(cm8)
```

```{r, echo=FALSE, warning=FALSE}
er1 <- errorest(mod1, data=df1, model=bagging, estimator="cv")
er2 <- errorest(mod1, data=df2, model=bagging, estimator="cv")
er3 <- errorest(mod1, data=df5, model=bagging, estimator="cv")
er4 <- errorest(mod1, data=df, model=bagging, estimator="cv")

er5 <- errorest(mod2, data=df1, model=bagging, estimator="cv")
er6 <- errorest(mod2, data=df2, model=bagging, estimator="cv")
er7 <- errorest(mod2, data=df5, model=bagging, estimator="cv")
er8 <- errorest(mod2, data=df, model=bagging, estimator="cv")
```

```{r, echo=FALSE}
btree1 <- append(btree1, er1$error)
btree2 <- append(btree2, er2$error)
btree3 <- append(btree3, er3$error)
btree4 <- append(btree4, er4$error)
btree5 <- append(btree5, er5$error)
btree6 <- append(btree6, er6$error)
btree7 <- append(btree7, er7$error)
btree8 <- append(btree8, er8$error)
```

```{r, echo=FALSE, warning=FALSE}
er1 <- errorest(mod1, data=df1, model=bagging, estimator="boot")
er2 <- errorest(mod1, data=df2, model=bagging, estimator="boot")
er3 <- errorest(mod1, data=df5, model=bagging, estimator="boot")
er4 <- errorest(mod1, data=df, model=bagging, estimator="boot")

er5 <- errorest(mod2, data=df1, model=bagging, estimator="boot")
er6 <- errorest(mod2, data=df2, model=bagging, estimator="boot")
er7 <- errorest(mod2, data=df5, model=bagging, estimator="boot")
er8 <- errorest(mod2, data=df, model=bagging, estimator="boot")
```
...
```{r, echo=FALSE}
btree_statistics <- data.frame(btree1, btree2, btree3, btree4, btree5, btree6, btree7, btree8)
btree_statistics <- round(btree_statistics,2)
btree_statistics <- cbind(name=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "10-fold CV error", "Bootstrap error"), btree_statistics)
```
...
```{r, echo=FALSE}
boost.1 <- confusion.matrix.results(cm1)
boost.2 <- confusion.matrix.results(cm2)
boost.3 <- confusion.matrix.results(cm3)
boost.4 <- confusion.matrix.results(cm4)
boost.5 <- confusion.matrix.results(cm5)
boost.6 <- confusion.matrix.results(cm6)
boost.7 <- confusion.matrix.results(cm7)
boost.8 <- confusion.matrix.results(cm8)
```

```{r, echo=FALSE, warning=FALSE}
my.ada <- function(formula1, data1, nbagg1, minsplit1) ada(formula=formula1, data=data1, iter=10)

er1 <- errorest(mod1, data=df1, model=my.ada, estimator="cv")
er2 <- errorest(mod1, data=df2, model=my.ada, estimator="cv")
er3 <- errorest(mod1, data=df5, model=my.ada, estimator="cv")
er4 <- errorest(mod1, data=df, model=my.ada, estimator="cv")

er5 <- errorest(mod2, data=df1, model=my.ada, estimator="cv")
er6 <- errorest(mod2, data=df2, model=my.ada, estimator="cv")
er7 <- errorest(mod2, data=df5, model=my.ada, estimator="cv")
er8 <- errorest(mod2, data=df, model=my.ada, estimator="cv")
```

```{r, echo=FALSE}
boost.1 <- append(boost.1, er1$error)
boost.2 <- append(boost.2, er2$error)
boost.3 <- append(boost.3, er3$error)
boost.4 <- append(boost.4, er4$error)
boost.5 <- append(boost.5, er5$error)
boost.6 <- append(boost.6, er6$error)
boost.7 <- append(boost.7, er7$error)
boost.8 <- append(boost.8, er8$error)
```

```{r, echo=FALSE, warning=FALSE}
er1 <- errorest(mod1, data=df1, model=my.ada, estimator="boot")
er2 <- errorest(mod1, data=df2, model=my.ada, estimator="boot")
er3 <- errorest(mod1, data=df5, model=my.ada, estimator="boot")
er4 <- errorest(mod1, data=df, model=my.ada, estimator="boot")

er5 <- errorest(mod2, data=df1, model=my.ada, estimator="boot")
er6 <- errorest(mod2, data=df2, model=my.ada, estimator="boot")
er7 <- errorest(mod2, data=df5, model=my.ada, estimator="boot")
er8 <- errorest(mod2, data=df, model=my.ada, estimator="boot")
```
...
```{r, echo=FALSE}
boost_statistics <- data.frame(boost.1, boost.2, boost.3, boost.4, boost.5, boost.6, boost.7, boost.8)
boost_statistics <- round(boost_statistics,2)
boost_statistics <- cbind(name=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "10-fold CV error", "Bootstrap error"), boost_statistics)
```
...
We can use both numeric and categorical data. Let's specify model formula. We use all attributes and also model without variables that are useless from EDA (Age, Steroid, AlkPhosphate, Sgot). Also we need number of features.
...
cm8 <- confusionMatrix(table(pred.labels8, real.labels), positive = "1")
cm9 <- confusionMatrix(table(pred.labels9, real.labels), positive = "1")
cm10 <- confusionMatrix(table(pred.labels10, real.labels), positive = "1")
cm11 <- confusionMatrix(table(pred.labels11, real.labels), positive = "1")
cm12 <- confusionMatrix(table(pred.labels12, real.labels), positive = "1")
```

```{r, echo=FALSE}
rf.1 <- confusion.matrix.results(cm1)
rf.2 <- confusion.matrix.results(cm2)
rf.3 <- confusion.matrix.results(cm3)
rf.4 <- confusion.matrix.results(cm4)
rf.5 <- confusion.matrix.results(cm5)
rf.6 <- confusion.matrix.results(cm6)
rf.7 <- confusion.matrix.results(cm7)
rf.8 <- confusion.matrix.results(cm8)
rf.9 <- confusion.matrix.results(cm9)
rf.10 <- confusion.matrix.results(cm10)
rf.11 <- confusion.matrix.results(cm11)
rf.12 <- confusion.matrix.results(cm12)
```

```{r, echo=FALSE, warning=FALSE}
my.randomForest1 <- function(formula1, data1) randomForest(formula=formula1, data=data1, ntree = 500, mtry = 19)
my.randomForest2 <- function(formula1, data1) randomForest(formula=formula1, data=data1, ntree = 500, mtry = sqrt(19))
my.randomForest3 <- function(formula1, data1) randomForest(formula=formula1, data=data1, ntree = 500, mtry = 15)
my.randomForest4 <- function(formula1, data1) randomForest(formula=formula1, data=data1, ntree = 500, mtry = sqrt(15))

er1 <- errorest(mod1, data=df1, model=my.randomForest1, estimator="cv")
er2 <- errorest(mod1, data=df2, model=my.randomForest1, estimator="cv")
er3 <- errorest(mod1, data=df5, model=my.randomForest1, estimator="cv")

er4 <- errorest(mod1, data=df1, model=my.randomForest2, estimator="cv")
er5 <- errorest(mod1, data=df1, model=my.randomForest2, estimator="cv")
er6 <- errorest(mod1, data=df5, model=my.randomForest2, estimator="cv")

er7 <- errorest(mod2, data=df1, model=my.randomForest3, estimator="cv")
er8 <- errorest(mod2, data=df2, model=my.randomForest3, estimator="cv")
er9 <- errorest(mod2, data=df5, model=my.randomForest3, estimator="cv")

er10 <- errorest(mod2, data=df1, model=my.randomForest4, estimator="cv")
er11 <- errorest(mod2, data=df2, model=my.randomForest4, estimator="cv")
er12 <- errorest(mod2, data=df5, model=my.randomForest4, estimator="cv")
```

```{r, echo=FALSE}
rf.1 <- append(rf.1, er1$error)
rf.2 <- append(rf.2, er2$error)
rf.3 <- append(rf.3, er3$error)
rf.4 <- append(rf.4, er4$error)
rf.5 <- append(rf.5, er5$error)
rf.6 <- append(rf.6, er6$error)
rf.7 <- append(rf.7, er7$error)
rf.8 <- append(rf.8, er8$error)
rf.9 <- append(rf.9, er9$error)
rf.10 <- append(rf.10, er10$error)
rf.11 <- append(rf.11, er11$error)
rf.12 <- append(rf.12, er12$error)
```

```{r, echo=FALSE, warning=FALSE}
er1 <- errorest(mod1, data=df1, model=my.randomForest1, estimator="boot")
er2 <- errorest(mod1, data=df2, model=my.randomForest1, estimator="boot")
er3 <- errorest(mod1, data=df5, model=my.randomForest1, estimator="boot")

er4 <- errorest(mod1, data=df1, model=my.randomForest2, estimator="boot")
er5 <- errorest(mod1, data=df1, model=my.randomForest2, estimator="boot")
er6 <- errorest(mod1, data=df5, model=my.randomForest2, estimator="boot")

er7 <- errorest(mod2, data=df1, model=my.randomForest3, estimator="boot")
er8 <- errorest(mod2, data=df2, model=my.randomForest3, estimator="boot")
er9 <- errorest(mod2, data=df5, model=my.randomForest3, estimator="boot")

er10 <- errorest(mod2, data=df1, model=my.randomForest4, estimator="boot")
er11 <- errorest(mod2, data=df2, model=my.randomForest4, estimator="boot")
er12 <- errorest(mod2, data=df5, model=my.randomForest4, estimator="boot")
```
...
```{r, echo=FALSE}
rf_statistics <- data.frame(rf.1, rf.2, rf.3, rf.4, rf.5, rf.6, rf.7, rf.8, rf.9, rf.10, rf.11, rf.12)
rf_statistics <- round(rf_statistics,2)
rf_statistics <- cbind(name=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "10-fold CV error", "Bootstrap error"), rf_statistics)
```
...
```{r, echo=FALSE}
all_statistics <- data.frame(append(lr.imp.1[-7],c(NA,NA)), append(lda.pb.1[-7],c(NA,NA)),append(qda.pb.1[-7],c(NA,NA)), logit4, append(knn.pb.1[-7],c(NA,NA)), tree6, btree6, boost.6, rf.10)
all_statistics <- round(all_statistics,2)
all_statistics <- cbind(name=c("Accuracy", "Kappa", "Sensitivity", "Specificity", "F1", "Balanced Accuracy", "10-fold CV error", "Bootstrap error"), all_statistics)
colnames(all_statistics) <- c("metric","Linear regression","LDA","QDA","Logistic regression","KNN","Random Tree", "Bagging Tree", "Boosted Tree", "Random Forest")
```
